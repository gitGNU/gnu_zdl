...
-d,     --daemon        Avvia ZDL in modalità \"demone\"
                        (può essere controllato attraverso l'interfaccia interattiva)
...
\e[1mZDL può essere avviato in diversi modi:\e[0m
	...
	2) Utilizzando uno o più file preparati con un editor di testi (andare a capo dopo ogni link) e raggiungibili dalla directory di destinazione (indica un path valido):
		- apri un terminale ed entra nella directory che dovrà contenere i file scaricati
    ...
    \e[1mB) PER GESTIRE DOWNLOAD GIÀ AVVIATI NELLE MODALITÀ \"NON INTERATTIVA\" E \"DEMONE\"\e[0m

        - Avviando la modalità interattiva in due modi:
                1) in console dalla directory di destinazione dei download, con il comando \e[1mzdl -i\e[0m oppure \e[1mzdl --interactive\e[0m
                2) nella modalità \"non interattiva\" (standard), digitando il tasto \"i\"

\e[1mZDL può essere fermato in diversi modi:\e[0m

        - Se i download sono gestiti dalla modalità \"non interattiva\"/standard:
                1) digitando Control+c (in questo caso saranno interrotti anche i download di Axel)
                2) digitando il tasto \"q\" (tutti i download già avviati nella directory corrente con Wget e Axel non saranno interrotti, ma non verranno più gestiti da ZDL)
        - Se i download sono gestiti dalla modalità \"demone\": 
                attraverso la modalità interattiva (\e[1mzdl -i\e[0m) avviata nella directory gestita dal demone, digitando il tasto di stop: \"s\" (in questo caso i download avviati con Axel e Wget non sarano interrotti, ma non verranno gestiti da ZDL)
...
Configurazione manuale di Flashgot per ZDL:
	...
	3)	ancora nel tab \"Generale\", il \"Percorso dell'eseguibile:\" 
			...
			su WINDOWS: [unità]\Cygwin\zdl.bat
...
ZDL implementa automaticamente il browser Conkeror con la nuova funzione \"zdl\". 
Per attivarla, digita: \e[1mM+x zdl\e[0m

Dopo aver avviato il comando, il minibuffer di Conkeror:
 - chiede di selezionare la \e[1mdirectory\e[0m di destinazione
 - offre la possibilità di utilizzare le \e[1mopzioni\e[0m di zdl (lasciare il campo vuoto per non selezionarle)
 - mostra e indicizza i \e[1mlink\e[0m nel buffer corrente (per esempio la pagina web), per procedere nella selezione del link (il link della pagina corrente corrisponde all'indice 0)
 - avvia ZDL in un terminale xterm oppure, se è stata selezionata l'opzione --daemon (oppure -d), in background. In ogni caso è possibile gestire i download utilizzando l'interfaccia interattiva: \e[1mzdl -i\e[0m



\e[1mXXXTerm (Xombrero) e altri software.\e[0m

ZDL dispone anche dello script \e[1m/usr/local/bin/zdl-xterm\e[0m, che può essere avviato da qualunque applicazione e da un terminale. È stato pensato per rendere possibile l'avvio di ZDL in contesti diversi da quelli previsti dal programma. In particolare, XXXTerm può avviare uno script bash, utilizzando il comando \e[1m:run_script\e[0m, passandogli un solo argomento: il link della pagina corrente. Questo rende impossibile l'avvio standard di zdl in un'istanza di xterm, perché \e[1m:run_script\e[0m non accetta ulteriori parametri. L'avvio di zdl in un nuovo terminale virtuale xterm è reso possibile da \e[1mzdl-xterm\e[0m.

\e[1mUso in XXXTerm:\e[0m
- comando \e[1m:run_script\e[0m
- digita: \e[1mzdl-xterm\e[0m
- il programma avvia un'istanza di xterm che:
   - chiede in quale \e[1mdirectory\e[0m scaricare il file (è possibile evitare questa interazione configurando zdl-xterm: inserisci in \e[1m/usr/local/bin/zdl-xterm\e[0m, come valore della variabile \e[1m\$default_directory\e[0m all'inizio dello script, il path di una \e[1mdirectory valida\e[0m)
   - zdl-xterm avvia zdl sull'\e[1mURL corrente\e[0m

\e[1mSemplificazione per XXXTerm.\e[0m
Per utilizzare zdl-xterm con più rapidità, XXXTerm può essere configurato in molti modi.
...
Con la configurazione suggerita, digitando \e[1mCtrl+z\e[0m si avvia il prompt di \e[1m:run_script\e[0m con il campo già occupato dal nome dello script \e[1mzdl-xterm\e[0m: è sufficiente premere \e[1minvio\e[0m.
...
Installazione su Windows in due fasi:
	FASE 1 _ Installazione di Cygwin 32 bit
		- installatore automatico di Cygwin (serve anche ad aggiornare il sistema emulato e ad installare nuovi pacchetti): http://cygwin.com/setup_x86.exe
		- con l'installatore di Cygwin, installa il pacchetto \"Wget\"

	FASE 2 _ Installazione di ZDL
		- salva nella cartella \\\cygwin il seguente file: http://inventati.org/zoninoz/html/upload/files/install_zdl.sh
